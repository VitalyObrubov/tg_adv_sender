...
async def adv_send(client: TelegramClient = None):
    start_time = datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    bot_config = BotConfig()
    sender_config = SenderConfig()
    if not client:
        client = TelegramClient('session_name_adv', bot_config.api_id, bot_config.api_hash)
        await client.start()
    errors = []
    list_post = None
    adv_post = None
...
if __name__ == '__main__':
    ...
    loop.run_until_complete(adv_send())
    #asyncio.run(adv_send())
    print("Finised")